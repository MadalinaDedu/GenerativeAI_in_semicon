...
//
//  ------------------------------------------------------------------------------------------------------
//  Description         :  RTL module for dual clock fifo
//  ======================================================================================================
...
module dual_clock_fifo #(parameter DATA_WIDTH = 8, parameter ADDR_WIDTH = 4)(
    ...
    always @(posedge wr_clk or negedge reset) begin
        if (!reset) wr_ptr <= 0         ;  else                              // Reset write pointer on reset
        if (wr_en ) wr_ptr <= wr_ptr + 1;                                    // Increment write pointer if write enable is active
    end

    // Write Domain: Convert write pointer to Gray code
    always @(posedge wr_clk or negedge reset)
        if (!reset) wr_ptr_gray <= 0                                 ; else  // Reset Gray code pointer on reset
                    wr_ptr_gray <= ((wr_ptr + 1) >> 1) ^ (wr_ptr + 1);       // Convert binary write pointer to Gray code


    // Read Domain: Increment read pointer on read clock edge
    always @(posedge rd_clk or negedge reset) begin
        if (!reset) rd_ptr <= 0         ;  else                              // Reset read pointer on reset
        if (rd_en ) rd_ptr <= rd_ptr + 1;                                    // Increment read pointer if read enable is active
    end

    // Read Domain: Convert read pointer to Gray code
    always @(posedge rd_clk or negedge reset) begin
        if (!reset) rd_ptr_gray <= 0                                 ; else  // Reset Gray code pointer on reset
                    rd_ptr_gray <= ((rd_ptr + 1) >> 1) ^ (rd_ptr + 1);       // Convert binary read pointer to Gray code
    end

    // Synchronize write pointer Gray code to read clock domain
    always @(posedge rd_clk or negedge reset) begin
        if (!reset) wr_ptr_gray_sync1 <= 0          ; else                   // Reset synchronized Gray code pointer on reset
    ...
    always @(posedge wr_clk or negedge reset) begin
        if (!reset) rd_ptr_gray_sync1 <= 0          ; else                   // Reset synchronized Gray code pointer on reset
    ...
    always @(posedge wr_clk)
        if (wr_en) fifo_mem[wr_ptr[ADDR_WIDTH-1:0]] <= data_in;              // Write data to FIFO memory at the address specified by write pointer


    // // Read Domain: Read data from FIFO memory
    always @(posedge rd_clk or negedge reset) begin
        if (!reset) data_out <= 0                               ; else       // Reset output data on reset
        if (rd_en ) data_out <= fifo_mem[rd_ptr[ADDR_WIDTH-1:0]];            // Read data from FIFO memory at the address specified by read pointer
